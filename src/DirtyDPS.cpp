...
const int32_t DirtyDPS::scaling_facts[DPS__NUM_OF_SCAL_FACTS] = {
...
int16_t DirtyDPS::getContResults(float *tempBuffer, uint8_t &tempCount,
...
int16_t DirtyDPS::startMeasureTempCont(uint8_t measureRate,
...
int16_t DirtyDPS::startMeasurePressureCont(uint8_t measureRate,
                                           uint8_t oversamplingRate) {
  // abort if initialization failed
  if (m_initFail) {
    return DPS__FAIL_INIT_FAILED;
...
int16_t DirtyDPS::startMeasureBothCont(uint8_t tempMr, uint8_t tempOsr,
...
int16_t DirtyDPS::setTempSource(TempSource sensor) {
  if (m_initFail) {
    return DPS__FAIL_INIT_FAILED;
  }
  uint8_t desired = (sensor == TEMP_SRC_AUTO)
                        ? m_tempSensorRec
                        : (static_cast<uint8_t>(sensor) & 0x01);
  m_tempSensor = desired;
  // Apply immediately to device
  int16_t ret = writeByteBitfield(desired, registers[TEMP_SENSOR]);
  if (ret != DPS__SUCCEEDED) {
    return DPS__FAIL_UNKNOWN;
  }
  return DPS__SUCCEEDED;
...
int16_t DirtyDPS::measureTempOnce(float &result, TempSource sensor) {
  return measureTempOnce(result, sensor, m_tempOsr);
}
...
int16_t DirtyDPS::measureTempOnce(float &result, TempSource sensor,
  ...
  // Save and temporarily override default temp sensor
  uint8_t prevDefault = m_tempSensor;
  uint8_t desired = (sensor == TEMP_SRC_AUTO)
                        ? m_tempSensor
                        : (static_cast<uint8_t>(sensor) & 0x01);
  m_tempSensor = desired;

  // If OSR is unchanged, configTemp won't be called, so explicitly set register
  if (oversamplingRate == m_tempOsr) {
    if (writeByteBitfield(desired, registers[TEMP_SENSOR]) != DPS__SUCCEEDED) {
      m_tempSensor = prevDefault;
      return DPS__FAIL_UNKNOWN;
    }
  }

  // Tell calcTemp which sensor this read belongs to
  m_currentTempSensor = desired;

  int16_t ret = measureTempOnce(result, oversamplingRate);

  // Restore default and device TEMP_SENSOR register
  m_tempSensor = prevDefault;
  writeByteBitfield(prevDefault, registers[TEMP_SENSOR]);
...
int16_t DirtyDPS::measureTempOnce(float &result, TempSource sensor,
  ...
  int16_t ret = measureTempOnce(result, oversamplingRate, transform);
...
int16_t DirtyDPS::readByteSPI(uint8_t regAddress) {
  ...
  m_spibus->beginTransaction(
      SPISettings(DPS3xx__SPI_MAX_FREQ, MSBFIRST, SPI_MODE3));
...
int16_t DirtyDPS::writeByteSpi(uint8_t regAddress, uint8_t data,
                               uint8_t check) {
  // this function is only made for communication via SPI
  if (m_SpiI2c != 0) {
    return DPS__FAIL_UNKNOWN;
...
int16_t DirtyDPS::writeByteBitfield(uint8_t data, uint8_t regAddress,
...
int16_t DirtyDPS::readBlock(RegBlock_t regBlock, uint8_t *buffer) {
  ...
  int16_t ret = m_i2cbus->requestFrom(m_slaveAddress, (uint8_t)regBlock.length,
                                      (uint8_t)1);
