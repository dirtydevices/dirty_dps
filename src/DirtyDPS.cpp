...
DirtyDPS::~DirtyDPS(void) { end(); }

void DirtyDPS::begin(TwoWire &bus) { begin(bus, DPS__STD_SLAVE_ADDRESS); }
...
void DirtyDPS::begin(SPIClass &bus, int32_t chipSelect) { begin(bus, chipSelect, 0U); }
...
void DirtyDPS::end(void) { standby(); }

////////  Public helpers  ////////

uint8_t DirtyDPS::getProductId(void) { return m_productID; }

uint8_t DirtyDPS::getRevisionId(void) { return m_revisionID; }

int16_t DirtyDPS::getContResults(float *tempBuffer, uint8_t &tempCount, float *prsBuffer, uint8_t &prsCount) {
    if (m_initFail) {
...
int16_t DirtyDPS::measureTempOnce(float &result) { return measureTempOnce(result, m_tempOsr); }
...
int16_t DirtyDPS::measureTempOnce(float &result, float &rawResult, uint8_t oversamplingRate) {
    ...
    ret = getSingleResult(result, rawResult);
    if (ret != DPS__SUCCEEDED) {
        standby();
    }
    return ret;
...
int16_t DirtyDPS::startMeasureTempOnce(void) { return startMeasureTempOnce(m_tempOsr); }
...
int16_t DirtyDPS::startMeasureTempOnce(uint8_t oversamplingRate) {
    ...
    // Record which temperature sensor will produce this reading
    m_currentTempSensor = m_tempSensor;

    // set device to temperature measuring mode
    return setOpMode(CMD_TEMP);
}

int16_t DirtyDPS::measurePressureOnce(float &result) { return measurePressureOnce(result, m_prsOsr); }
...
int16_t DirtyDPS::startMeasurePressureOnce(void) { return startMeasurePressureOnce(m_prsOsr); }
...
int16_t DirtyDPS::startMeasureTempCont(uint8_t measureRate, uint8_t oversamplingRate) {
    ...
    // Record which temperature sensor will produce these readings
    m_currentTempSensor = m_tempSensor;
...
int16_t DirtyDPS::startMeasureBothCont(uint8_t tempMr, uint8_t tempOsr, uint8_t prsMr, uint8_t prsOsr) {
    // abort if initialization failed
    if (m_initFail) {
        return DPS__FAIL_INIT_FAILED;
    }
    ...
    // Record which temperature sensor will produce temperature readings
    m_currentTempSensor = m_tempSensor;
    // Start measuring in background mode
...
int16_t DirtyDPS::getIntStatusFifoFull(void) { return readByteBitfield(config_registers[INT_FLAG_FIFO]); }

int16_t DirtyDPS::getIntStatusTempReady(void) { return readByteBitfield(config_registers[INT_FLAG_TEMP]); }

int16_t DirtyDPS::getIntStatusPrsReady(void) { return readByteBitfield(config_registers[INT_FLAG_PRS]); }

int16_t DirtyDPS::flushFIFO() { return writeByteBitfield(1U, registers[FIFO_FL]); }

int16_t DirtyDPS::enableFIFO() { return writeByteBitfield(1U, config_registers[FIFO_EN]); }
...
int16_t DirtyDPS::writeByte(uint8_t regAddress, uint8_t data) { return writeByte(regAddress, data, 0U); }
...
int16_t DirtyDPS::writeByteBitfield(uint8_t data, RegMask_t regMask) { return writeByteBitfield(data, regMask.regAddress, regMask.mask, regMask.shift, 0U); }

int16_t DirtyDPS::writeByteBitfield(uint8_t data, uint8_t regAddress, uint8_t mask, uint8_t shift, uint8_t check) {
...
void DirtyDPS::init(void) {
    ...
    m_tempSensorRec = static_cast<uint8_t>(sensor) & 0x01;
    m_tempSensor = m_tempSensorRec;
    m_currentTempSensor = m_tempSensor;
    // initialize per-sensor scaled temperature caches
    m_lastTempScalBySensor[0] = 0.0f;
    m_lastTempScalBySensor[1] = 0.0f;
    m_lastTempScal = 0.0f;
